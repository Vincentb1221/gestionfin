...
suggestions = ["AAPL", "MSFT", "TSLA", "GOOGL", "AMZN", "VTI", "SPY", "XIC.TO"]
...
# Section 2 : Portefeuille
    ...
    with st.form(key="ajout_actif"):
        ...
        with col1:
            symbole = st.selectbox("Symbole (ou entrez le nom)", options=[""] + suggestions, index=0)
            saisie_libre = st.text_input("Ou saisissez un nom ou symbole")
        with col2:
            type_actif = st.selectbox("Type", ["Actions", "Obligations", "FNB"])
        ...
        symbole_final = saisie_libre.strip() if saisie_libre else symbole

        if submit and symbole_final:
            try:
                symbole_final = trouver_symbole(symbole_final)
                ...
                new_row = {"Actif": symbole_final.upper(), "Type": type_actif, "Quantité": quantite, 
                ...
                st.success(f"{symbole_final.upper()} ajouté au portefeuille !")
    ...
    if not st.session_state.portefeuille.empty:
        ...
        total_portefeuille = portefeuille_complet["Valeur Totale"].sum()
        st.metric("💼 Valeur totale du portefeuille", f"${total_portefeuille:,.2f}")

        repartition = portefeuille_complet.groupby("Type")["Valeur Totale"].sum()
        fig, ax = plt.subplots()
        ax.pie(repartition, labels=repartition.index, autopct='%1.1f%%', startangle=90)
        ax.axis('equal')
        st.pyplot(fig)

        actif_a_supprimer = st.selectbox("Supprimer un actif", portefeuille_complet["Actif"].tolist())
